...
| Effects | **Workflow lane UNIFIED by agreement proof** — the live receipt seam (workflow-call-receipts) already enforces the contract's machine in production; `effect-contract-agreement.test.ts` pins the two together per state, so divergence fails a test (the planWorkflowExecutionBatches precedent). Chat-lane tool boundary migrates in a later slice | 10 crash-window pins + 4 agreement pins |
| Provider lifecycle | **Contract landed** (`provider-fallover.ts`) — fallover as a total function over effect ledgers (ambiguity observes first, no override arm), work identity as three digests with target text structurally absent; brain seams migrate onto it in the Stage 8 slices | 8 pins |
