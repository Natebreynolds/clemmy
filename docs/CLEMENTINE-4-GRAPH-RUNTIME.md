...
Local v3.6 candidate validation is green on the combined working tree: the integrated graph/demo/effect/sales acceptance passed 80/80; the full isolated repository corpus passed 8,280 with zero failures and one existing skip; journeys passed 5/5; public-hygiene tests passed 3/3; release-asset tests passed 40/40; unified report-back smoke passed 7/7; proof self-tests passed 53/53; fresh-install smoke passed; and daemon, mobile-web, console-web, and desktop builds completed. The source HEAD moved once during this gate, so these commands must be rerun from the final candidate commit before it becomes the freeze point. Live both-brain/provider proof and the controlled cloned-workflow canary are deliberately not inferred from unit tests.
