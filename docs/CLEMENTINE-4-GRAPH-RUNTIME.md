...
Status: Horizon A implemented; Horizon B pilot 1 implemented behind explicit graph semantics and shadow observation; local gates and stable-commit Codex/Claude critical and endurance proofs are green on runtime candidate `0fbf2323`; a controlled cloned-workflow canary remains
Date: 2026-08-01
...
Worker-result reuse is bound to work identity, never target text alone. Manifest reuse requires the exact manifest, contract version, phase, and canonical item; non-manifest replay requires the exact execution-packet key. A later contract revision overrides conflicting original or earlier requirements while preserving only compatible requirements and completed evidence. This is a graph identity invariant: the same target under a different phase, operation, or contract is new work.
...
Local v3.6 candidate validation is green from immutable runtime candidate `0fbf2323`: the release-focused graph/demo/effect/sales gate passed 167/167; the full isolated repository corpus ran 8,266 tests with 8,265 pass, zero failures, and one deliberate platform-branch skip; and the conditional console/runner/daemon set passed 42/42 after builds. Journeys passed 5/5, public-hygiene tests passed 3/3, release-asset tests passed 40/40, unified report-back smoke passed 7/7, proof self-tests passed 53/53, fresh-install smoke passed, and root, mobile-web, console-web, and desktop typechecks/builds completed. Stable-commit proof on the same runtime candidate passed all five critical scenarios on Codex (93/93 plus exact-brain-served) and Claude (93/93 plus exact-brain-served). Each 120-item endurance run passed 21/21 with 120/120 fresh worker starts, 240 lifecycle checkpoints, zero target-only reuse, one terminal, and no narration leak. The steering proof required 48 real starts and a revised dispatch/output containing `REVALIDATED` without the superseded `BASELINE` marker. An initial parity audit exposed and rejected target-only reuse that could falsely satisfy a revised contract; the final candidate removes that shortcut. These proofs used disposable homes and proof-local provider shims. Private-fixture provenance, exact live demo replay across every claimed brain/surface, a controlled cloned-workflow canary, and real connected-account deployment effects are deliberately not inferred from them.
