/**
 * Shared humanizers for tool calls, so a tool call reads the same on every
 * surface — "outlook send email → paul@…", never a raw
 * "composio_execute_tool" or a synthetic "reflection end" row.
 *
 * Canonical copy for both UIs (source of the desktop version at
 * apps/console-web/src/lib/toolLabels.ts until it adopts this package).
 */
 ...
 * actions and must never surface in a user-facing tool feed.
 */
 ...
 *  never an ordered slug-regex chain. Mirrors the server's describeExternalWrite
 *  (src/runtime/harness/work-report.ts) so the live feed and the report-back
 *  message read identically; a parity test pins the copies phrase-identical.
 *  A write recorded reversible may never render as delivery. */
 ...
 *  plane attaches (raw args are private there) — it wins when present. */
