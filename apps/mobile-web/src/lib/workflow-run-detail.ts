/**
 * VERBATIM COPY of apps/console-web/src/lib/workflow-run-detail.ts — the
 * desktop's pure run-timeline reducer. Zero imports, no React, no I/O, so it
 * ports as-is; keep the two copies identical until both apps consume it from
 * a shared package (the console file was mid-edit in another work stream when
 * this copy was made, so extraction waits for that ring to close).
 */
/**
