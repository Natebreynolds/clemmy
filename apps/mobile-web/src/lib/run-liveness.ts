...
export function runIsLive(input: {
  ...
  if (input.stampedAt || input.stale) return false;
...
/** Waiting work may still be stopped. This does not grant authority: the exact
 * server run ID and cancellation route remain responsible for admission. */
export function runCanBeControlled(input: { status?: string | null; stampedAt: string | null; stale?: boolean }): boolean {
  return Boolean(input.status && !input.stampedAt && !input.stale && isActiveRunStatus(input.status));
}
