...
import { runElapsedLabel, runIsLive, runCanBeControlled } from './run-liveness';
...
test('a stamped copy of a running run is not live', () => {
  ...
  assert.equal(runIsLive({ status: 'awaiting_input', stampedAt: null }), false);
  assert.equal(
...
test('the run screen derives liveness, and offers Stop only on a live run', () => {
  ...
  assert.match(run, /const live = runIsLive\(\{ status: run\?\.status, stampedAt, stale: stale \|\| offline \|\| Boolean\(error\) \}\);/,
  ...
  assert.match(run, /\{controllable \? <RunControl key=\{run.id\} target=/, 'Stop is gated on the derived liveness');
  assert.match(run, /lastGood=\{lastGoodNotice\(stampedAt, Date\.now\(\)\)\}/,
    'the banner reads the same stamp the liveness gate does');
});

test('failed in-memory refresh cannot advertise live execution or a current Stop target', () => {
  assert.equal(runIsLive({ status: 'running', stampedAt: null, stale: true }), false);
  assert.equal(runCanBeControlled({ status: 'running', stampedAt: null, stale: true }), false);
  assert.equal(runIsLive({ status: 'awaiting_input', stampedAt: null }), false);
  assert.equal(runCanBeControlled({ status: 'awaiting_input', stampedAt: null }), true);
});
