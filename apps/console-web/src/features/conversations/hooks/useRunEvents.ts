...
function recentUrl(sessionId: string, sinceSeq: number): string {
  return `/api/sessions/${encodeURIComponent(sessionId)}/events/recent?sinceSeq=${sinceSeq}&limit=${PAGE_LIMIT}`;
}

/** Append only what this buffer has not already seen, in wire order. A re-read
 *  from `maxSeq` returns exactly the tail, so nothing is duplicated and nothing
 *  already shown is dropped. */
function appendEvents(buffer: StepBuffer, incoming: readonly HarnessEvent[]): number {
  let added = 0;
  for (const event of incoming) {
    const seq = typeof event.seq === 'number' ? event.seq : 0;
    if (seq > 0) {
      if (seq <= buffer.maxSeq) continue;
      buffer.maxSeq = seq;
    }
...
export function useRunEvents(session: RunSessionLike): RunEventsState {
  ...
  useEffect(() => {
    ...
    const bufferFor = (id: string): StepBuffer => {
      ...
      const fresh: StepBuffer = { events: [], maxSeq: 0 };
    ...
    const publishCoverage = () => {
      if (cancelled) return;
      setCoverage({ steps: stepIds.length, read: readSteps.size, truncated });
    };
    ...
    const readStep = async (id: string): Promise<{ status: string | null; sawFinal: boolean }> => {
      ...
      try {
        for (let page = 0; page < MAX_PAGES; page += 1) {
          const data = await apiGet<RecentEventsPage>(recentUrl(raw, buffer.maxSeq));
          if (cancelled) return { status: null, sawFinal };
          status = data.sessionStatus ?? status;
          const batch = data.events ?? [];
          if (batch.some((event) => endsTheRun(event.type))) sawFinal = true;
          if (appendEvents(buffer, batch) > 0) scheduleFlush();
          if (batch.length < PAGE_LIMIT) break;
          if (page === MAX_PAGES - 1) {
            truncated = typeof data.latestSeq === 'number' ? data.latestSeq > buffer.maxSeq : true;
          }
        }
      } catch (err) {
        noteFailure(err);
    ...
    const followStep = async (id: string) => {
      ...
      const handle = runHarnessStream(rawId(id), {
        ...
        onEvent: (event) => {
          if (cancelled) return;
          if (endsTheRun(event.type)) sawFinal = true;
          if (appendEvents(buffer, [event]) > 0) scheduleFlush();
        },
      });
      handles.push(handle);
      await handle.promise;
      if (cancelled) return;
      if (sawFinal) { await readStep(id); return; }
      while (!cancelled) {
