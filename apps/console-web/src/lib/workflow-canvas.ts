 ...
 * is drawn is what the engine compiles. Saving goes back through the existing
 * PATCH /api/console/workflows/:name `steps` body, which merges incoming steps
 * over the stored ones by id. The canvas therefore sends `{ id, dependsOn }`
 * and nothing else for a step it did not create: prompt, allowedTools, output
 * contracts, approval gates and every other authored field survive a save
 * untouched because the canvas never names them.
 *
 ...
 *
 * 2. The authoring path does not reject a dependency cycle. The PATCH route
 *    runs validateWorkflowStepGraph (duplicate ids + unknown deps) and not
 *    validateWorkflowGraph (which is the one that detects cycles, and is only
 *    reached at run time). A cycle drawn on a canvas would persist happily and
 *    then stall the run, so `findCycle` below is the guard that has to stop it,
 *    and the editor refuses to save while it reports one.
 */
...
/**
 * The `steps` body for PATCH /api/console/workflows/:name.
 *
 * Edges are the authority for `dependsOn`, so disconnecting on the canvas
 * actually removes the dependency. Every node present is sent — a step left out
 * of the array is DELETED by the route, so an incomplete list would silently
 * drop steps. Order is topological with ties broken by the node's current
