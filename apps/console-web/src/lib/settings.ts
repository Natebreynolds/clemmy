...
// ─── Licensing ───────────────────────────────────────────────────────────────
// Mirrors LicensePosture in src/licensing/license-status.ts. The five states
// exist to keep "never had a key" apart from "no longer licensed"; the card
// renders them very differently and must never conflate the two.
export type LicenseState = 'unlicensed' | 'active' | 'stale' | 'expired' | 'revoked';
export type LicenseCheckOutcome = 'ok' | 'rejected' | 'unreachable';
...
export interface LicenseStatus {
  ...
  ok: boolean;
  /** Whether the SERVER has asked us to enforce. */
  enforcing: boolean;
  plan: string | null;
  features: string[];
  seat: { used: number; limit: number } | null;
  expiresAt: string | null;
  /** When we last ATTEMPTED a check — success is lastCheckOutcome. */
  lastCheckAt: string | null;
  lastCheckOutcome: LicenseCheckOutcome | null;
  gaps: LicenseGap[];
...
export interface LicenseRefreshResult extends LicenseStatus {
  tick?: { ran: boolean; outcome?: LicenseCheckOutcome; reason?: string };
}

export const getLicenseStatus = () => apiGet<LicenseStatus>('/api/console/license');
export const refreshLicense = () => apiPost<LicenseRefreshResult>('/api/console/license/refresh');
