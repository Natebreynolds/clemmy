...
export function endsTheRun(event: string | { seq?: number; type: string; data?: Record<string, unknown> }): boolean {
  if (typeof event !== 'string' && readLiveApprovalControl({ ...event, seq: event.seq ?? 0 })) return false;
  const type = typeof event === 'string' ? event : event.type;
  if (!RUN_FINAL_EVENTS.has(type)) return false;
  if (type !== 'conversation_completed' || typeof event === 'string') return true;
  ...
  // The public projection carries the validated durable disposition. A
  // resumable presentation closes a stream, not the work we are observing.
  if (presentation?.resumable === true || outcome?.resumable === true) return false;
  if (presentation?.status === 'needs_input' || outcome?.status === 'needs_input') return false;
  if (presentation?.status && outcome?.status && presentation.status !== outcome.status) return false;
  return true;
}

/** A newer accepted turn clears the previous terminal observation. */
export function latestRunBoundary(events: readonly { seq?: number; type: string; data?: Record<string, unknown> }[]) {
  for (let i = events.length - 1; i >= 0; i -= 1) {
    const event = events[i]!;
    if (readLiveApprovalControl({ ...event, seq: event.seq ?? 0 })) continue;
    if (event.type === 'user_input_received' || event.type === 'tool_called') return null;
    if (RUN_FINAL_EVENTS.has(event.type)) return event;
  }
  return null;
}

export function observedRunStatus(events: readonly { seq?: number; type: string; data?: Record<string, unknown> }[], fallback: string): string {
  const boundary = latestRunBoundary(events);
  return boundary && !endsTheRun(boundary) ? 'paused' : fallback;
}
...
export interface RunSourceCoverage {
  version: 1;
  kind: 'known_steps';
...
export interface RunReadCoverage {
  ...
  truncated: boolean;
  /** Known step sessions do not establish coverage of every workflow effect. */
  knownStepsOnly?: boolean;
...
export function runCoverageNotice(coverage: RunReadCoverage): string | null {
  if (coverage.recordUnavailable) return 'The workflow record could not be read. Its current status and complete history are unavailable.';
  if (coverage.knownStepsOnly && coverage.read === coverage.steps && !coverage.truncated) {
    return 'Showing recorded step conversations. Other workflow activity may not appear here.';
  }
  ...
  if (coverage.truncated) {
    return 'The server has not confirmed complete run history. What is listed below may be only part of it.';
  }
  return null;
}
...
export function runLedgerIsComplete(run: RunLiveness, coverage: RunReadCoverage): boolean {
  return run.over
    ...
    && !coverage.truncated
    && !coverage.knownStepsOnly
    && !coverage.recordUnavailable;
}
...
export function runEmptyStateText(input: {
  ...
  if (input.coverage.truncated || input.coverage.knownStepsOnly || input.coverage.recordUnavailable) {
    return 'No events were found in the available history. This does not establish whether the run changed anything.';
  }
...
export function runFinalReply(events: readonly RunEventInput[]): string {
  for (let i = events.length - 1; i >= 0; i -= 1) {
    const ev = events[i];
    if (ev.type !== 'conversation_completed' || readLiveApprovalControl({ ...ev, seq: ev.seq ?? 0 })) continue;
