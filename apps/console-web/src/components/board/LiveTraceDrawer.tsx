...
/** Server view of one guest CLI run (GET /api/console/guest-runs/:id). */
interface GuestRunView {
  id: string;
  harness: string;
  projectName: string;
  projectPath: string;
...
export function LiveTraceDrawer({
  ...
  // Guest CLI runs (project_run): an external Claude Code / Codex process —
  // no harness session to stream, so the drawer polls the guest-run feed.
  const isGuest = card.sourceKind === 'guest';
  ...
  // Guest-run poll: cheap full-state fetch every 2s while running; a single
  // fetch renders the final state for completed cards.
  const guestRunQ = usePoll(
    ['guest-run', String(card.raw.guestRunId ?? '')],
    () => apiGet<GuestRunView>(`/api/console/guest-runs/${encodeURIComponent(String(card.raw.guestRunId))}`),
    2000,
    { enabled: isGuest && Boolean(card.raw.guestRunId) },
  );
  const guestRun = guestRunQ.data ?? null;
        ...
        <div className="px-5 py-4">
          {isCatchup ? (
            ...
            <pre className="whitespace-pre-wrap break-words rounded-sm bg-canvas p-3 text-caption text-muted">
              {JSON.stringify(isGuest ? (guestRun ?? {}) : isWorkflow ? rawWorkflow : rawHarness, null, 2)}
            </pre>
          ) : isWorkflow ? (
            ...
            <div className="space-y-3">
              <div className="rounded-md border border-border bg-surface px-4 py-3">
                <div className="text-small font-semibold text-fg">
                  {String(card.raw.harness ?? 'CLI')} working in {String(card.raw.projectName ?? 'project')}
                </div>
                <p className="mt-1 break-words font-mono text-caption text-muted">{String(card.raw.prompt ?? '')}</p>
              </div>
              <div className="rounded-sm bg-canvas p-3">
                {(guestRun?.events ?? []).length === 0 ? (
                  <p className="text-caption text-faint">
                    {guestRun ? 'No output yet — the CLI is starting up.' : 'Loading the run feed…'}
                  </p>
                ) : (
                  <ol className="space-y-1">
                    {(guestRun?.events ?? []).map((line, i) => (
                      <li key={i} className="break-words font-mono text-caption text-muted">{line}</li>
                    ))}
                  </ol>
                )}
              </div>
              {guestRun && guestRun.status !== 'running' && (
                <div className={cn(
                  'rounded-md border px-4 py-3',
                  guestRun.status === 'succeeded' ? 'border-success/30 bg-success-tint' : 'border-danger/30 bg-danger-tint',
                )}>
                  <div className={cn('text-small font-semibold', guestRun.status === 'succeeded' ? 'text-success' : 'text-danger')}>
                    {guestRun.status === 'succeeded' ? 'Finished' : guestRun.status === 'killed' ? 'Stopped' : 'Failed'}
                    {guestRun.durationMs ? ` · ${Math.round(guestRun.durationMs / 1000)}s` : ''}
                  </div>
                  {(guestRun.finalMessage || guestRun.error) && (
                    <p className="mt-1 whitespace-pre-wrap break-words text-caption text-fg">
                      {(guestRun.finalMessage || guestRun.error || '').slice(0, 2000)}
                    </p>
                  )}
                  {guestRun.changedFiles.length > 0 && (
                    <div className="mt-2">
                      <div className="text-caption font-semibold text-faint">Files created or changed</div>
                      <ul className="mt-0.5 space-y-0.5">
                        {guestRun.changedFiles.map((f) => (
                          <li key={f} className="break-words font-mono text-caption text-muted">{f}</li>
                        ))}
                      </ul>
                    </div>
                  )}
        ...
        <footer className="flex items-center justify-between gap-4 border-t border-border px-5 py-3">
          <span className="text-caption text-faint">
            {isCatchup
              ...
              : isGuest
                ? `${guestRun?.events.length ?? 0} events · ${guestRun?.status === 'running' ? 'polling' : guestRun?.status ?? 'loading'}`
                : isWorkflow
