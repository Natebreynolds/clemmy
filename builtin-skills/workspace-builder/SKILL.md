...
- For a view-only edit that does not change data interpretation, use `space_get` with `metadata_only: true` to check the exact saved sources, accounts, schedules and actions without pulling dataset records. Read records when you need their shape or content.
...
- **Shape a read without a model**: add `transforms_json`, a JSON array of `{id,transform:{version:1,expression:<workflow transform>}}` steps. `steps.read.output` is this source's successful read result (CLI stdout is already parsed); later steps can read `steps.<earlier-id>.output`. `input.observed_at` is the host's read timestamp. The last step becomes the source dataset for desktop and phone. This uses the same pure operations as workflow transforms, with at most 16 steps and 64 KiB of definitions. A shaping error preserves the prior dataset and reports a failed refresh. Keep the existing output shape when migrating a source. No calls, code, or other source access are available inside these steps.
...
- **Workflow-fed data**: a workflow publishes into a source with a `space_set_data` call step whose `data_json` comes from a transform step. Never have a model step re-type a dataset. For one row per entity, sort by preference first (`sort`, `by:[{column,direction:"desc"}]`), then use `unique` with entity `keys`; it keeps the first row. Sort is stable, uses scalar comparisons (ISO date strings sort chronologically), and puts explicit nulls last in either direction. Keys are literal row field names; absent/non-scalar keys fail instead of silently dropping records. String sorting uses UTF-16 code-unit order, not locale collation. Preserve the existing dataset shape for both desktop and phone.
