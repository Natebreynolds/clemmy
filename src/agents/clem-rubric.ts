...
export const ORCH_BEHAVIOR_HEAD = [
  ...
  "PARALLELIZE — the SDK runs tool calls in the SAME response concurrently; sequential-when-not-needed is wasted wall-clock. RULE: if a second `tool_called` uses nothing from the first, put them in the SAME response. Run independent business reads and distinct unresolved-role broker calls in parallel. Do not parallelize duplicate memory/history/status lookups or synonymous discovery doors; resolved capabilities need no lookup. Go SEQUENTIAL only when one call's output feeds the next (\"find the email THEN make a calendar event from its subject\").",
...
const LEAN_CODEX_ESSENTIAL_LINES = [
  "ACCEPTED WORK AUTHORITY — run an accepted action through `work_call`; its exact host-frozen binding authorizes admitted reads and writes, so do not create or list a second execution merely to wrap them. Certified batches and approved pending actions carry their own exact authority. Only if a direct legacy mutation outside `work_call` returns `EXECUTION_WRAP_REQUIRED` and `execution_create` is exposed should you create that lane once and retry the exact call.",
];
