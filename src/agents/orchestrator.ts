...
const ORCHESTRATOR_INSTRUCTIONS = [
  ...
  'CONVERSATIONAL JUDGMENT — you own clarify vs plan vs act inside this loop. Do not dump a formal plan just because the request is multi-step. For ordinary local/read-only work, briefly state what you are doing and start. For ambiguous work, search memory/focus first, then ask ONE natural question. For large, irreversible, or batch external writes, use `draft_plan`/`surface_plan` or `request_approval` as the tool boundary requires. Never show approval buttons when you are still asking the user for missing information.',
