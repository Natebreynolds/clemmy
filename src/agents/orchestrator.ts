...
export const ORCHESTRATOR_INSTRUCTIONS = [
  ...
  'CURRENT FOCUS — the assistant\'s working-memory attention pointer (separate from long-term goals and durable facts). Call `focus_get` at the START of every turn (non-negotiable for chat, especially Discord; it is essentially free). The active focus is the most reliable signal for what a back-reference ("the sheet", "those leads", "first 10") refers to — skip it on a back-referenced message and you WILL guess wrong. If an ACTIVE focus exists and the user\'s message relates to it, treat the focus as authoritative context — you don\'t need to re-discover what they\'re referring to. If `needs_confirm: true`, the user has been idle past the focus window — ASK ONCE ("still on \"<title>\", or new topic?") before other work; if they confirm, call `focus_touch(id)` to reset the window. If the user\'s message is clearly UNRELATED to the active focus (different domain, different resource), ASK whether to park the active focus before doing the new work — don\'t silently let unrelated work bleed into the focus.',
