...
const ORCHESTRATOR_INSTRUCTIONS = [
  ...
  'External actions — DISCOVER FIRST, then EXECUTE. When the user wants an action on a connected external service (Instagram post, Slack DM, send an email, create a Trello card, etc.), use this pipeline:',
  '  1. Hand off to Researcher with a directive like "find the Composio tool for posting to Instagram, return the slug and required arguments." Researcher will call composio_search_tools and return the exact slug + arg schema. This is the discovery step.',
  '  2. With the slug in hand, call request_approval (since external mutations need user consent). Include the resolved tool slug in your decision summary so the next turn has the full context.',
  '  3. After approval, hand off to Executor with a CONCRETE directive that includes the discovered slug and the args. Example: "Use composio_execute_tool with slug `INSTAGRAM_POST_CREATE` and args {image_path: <...>, caption: <...>}." Executor will then call composio_execute_tool directly with that slug — NO redundant search.',
  'Skipping the discovery step (handing straight to Executor) makes the Executor either fall back to its own composio_search_tools (wasted turn + risk of picking the wrong slug) or fail outright if no curated cx_* matches. Discovery in Researcher → execution in Executor is the agentic pattern; cross those wires and the conversation slows down or breaks.',
  'EXCEPTION: if you already see a curated `cx_<toolkit>_<action>` tool that obviously matches (e.g. user says "send a Gmail draft" and you know cx_gmail_create_draft is a thing), you can skip the Researcher discovery step. Curated tools are the fast path. The discovery step is only required when a cx_* tool for the action doesn\'t exist.',
