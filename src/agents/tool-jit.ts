...
/**
 * The single source of truth for "does JIT run this turn?". The global CLEMMY_TOOL_JIT
 * flag governs, always respecting the lane gate (interactive chat only) — autonomous
 * lanes never JIT.
 */
export function resolveToolJitDecision(opts: { allowLane: boolean; sessionId?: string | null }): ToolJitDecision {
  return { active: opts.allowLane && toolJitEnabled() };
}
