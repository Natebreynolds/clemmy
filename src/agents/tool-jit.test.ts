...
import { selectToolsForTurn, toolJitEnabled, TOOL_JIT_CORE, TOOL_JIT_MANDATED, resolveToolJitDecision, type JitTool, type JitRankFn } from './tool-jit.js';
...
test('resolveToolJitDecision: lane gate always wins (autonomous lane never JITs)', () => {
  withEnv({ CLEMMY_TOOL_JIT: 'on' }, () => {
    assert.equal(resolveToolJitDecision({ allowLane: false, sessionId: 'sess-x' }).active, false);
  });
});

test('resolveToolJitDecision: global flag governs on the interactive lane', () => {
  withEnv({ CLEMMY_TOOL_JIT: 'on' }, () => {
    assert.equal(resolveToolJitDecision({ allowLane: true, sessionId: 'sess-x' }).active, true);
  });
  withEnv({ CLEMMY_TOOL_JIT: undefined }, () => {
    // default-on: unset global flag activates JIT.
    assert.equal(resolveToolJitDecision({ allowLane: true, sessionId: 'sess-x' }).active, true);
  });
  withEnv({ CLEMMY_TOOL_JIT: 'off' }, () => {
