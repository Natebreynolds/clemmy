...
export async function buildResearcherAgent(): Promise<SubAgent> {
  ...
  return new Agent<RuntimeContextValue>({
    ...
    instructions: harnessInstructions([
      ...
      'Pattern: form a query → call memory_recall (or memory_search if you have specific keywords) → read the most relevant hits → return a concise summary. If a project / file / decision is referenced and you find no match, say so explicitly so the orchestrator can decide whether to ask the user.',
