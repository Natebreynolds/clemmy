...
export async function buildWorkerAgent(options: { mcpToolScope?: McpToolScope } = {}): Promise<SubAgent> {
  ...
  return new Agent<RuntimeContextValue>({
    ...
    instructions: [
      ...
      '  - NEVER create a per-item artifact (email/Outlook draft, record, message) with BLANK or PLACEHOLDER identity fields. If your item is "draft an email to account X", the draft MUST carry X\'s real recipient address and a real first-name greeting from the data you were given or fetched. If a required identity field (recipient email, contact first name) is genuinely missing for your item and one retry to fetch it fails, do NOT produce a hollow draft — return "ERROR: missing <field> for <item>" so the parent can decide. A blank-recipient draft is a FAILED item, not a completed one.',
