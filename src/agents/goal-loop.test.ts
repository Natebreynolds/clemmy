...
test('runGoalLoop: judge says done on turn 1 → status=done', async () => {
...
test('runGoalLoop: budget exhausted without done → status=paused', async () => {
...
test('runGoalLoop: cancellation mid-loop → status=aborted', async () => {
...
test('describeGoalState: returns sensible strings for each status', () => {
  ...
  assert.match(goalMod.describeGoalState({ ...base, status: 'active' }), /active.*5\/20/);
  assert.match(goalMod.describeGoalState({ ...base, status: 'paused' }), /paused at 5\/20/);
  assert.match(goalMod.describeGoalState({ ...base, status: 'done' }), /complete/);
  assert.match(goalMod.describeGoalState({ ...base, status: 'aborted' }), /aborted/);
});

test('listActiveGoals: returns only active/paused goals', () => {
  // Add three goals across distinct sessionIds; vary their status.
  const ids = ['sess:list-1', 'sess:list-2', 'sess:list-3'];
  for (const id of ids) goalMod.clearGoalState(id);
  goalMod.saveGoalState({
    sessionId: 'sess:list-1', objective: 'a', status: 'active',
    startedAt: '', updatedAt: '', turnsUsed: 1, turnsLimit: 20, judgeParseFailures: 0,
  });
  goalMod.saveGoalState({
    sessionId: 'sess:list-2', objective: 'b', status: 'done',
