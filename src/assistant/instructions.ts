...
export function buildAssistantInstructions(context: MemoryContext, channel?: string, intent?: MessageIntent, message?: string): string {
  ...
  const planner = 'Keep clarify / plan / act conversational inside the main loop. Use `draft_plan` only for explicit planning requests, genuinely uncertain complex work, or when `CONFIRM_FIRST_REQUIRED` asks for a reviewed batch scope. Merely having many independent items is execution topology, not a reason to invoke a second planning model — use run_worker/workManifest and begin. If the returned plan has open `needsUserInput` questions, ask the user the shortest necessary clarification first; do NOT call `surface_plan` and do NOT show approval buttons for an incomplete plan. If the plan is executable and needs review, call `surface_plan` and stop until you see "Plan approved: <objective>". If the plan is moderate and safe/local/read-only, call `share_plan` and continue. Skip the Planner for trivial reads and natural conversation.';
