...
test('relay licensing: a lease must be valid, for this product, and bound to THIS Mac', async () => {
  // The teeth. A local check is deterrence — the daemon runs on the user's
  // machine and can be patched — so this pins the layer that actually holds.
  const { generateKeyPairSync } = await import('node:crypto');
  const { buildLeasePayload, signLease, publicKeyBase64 } = await import('../../apps/license/lease.mjs') as {
    buildLeasePayload: (o: Record<string, unknown>) => Record<string, unknown>;
    signLease: (p: unknown, o: { privateKeyPem: string; kid: string }) => string;
    publicKeyBase64: (pem: string) => string;
  };
  const { privateKey } = generateKeyPairSync('ed25519');
  const privateKeyPem = privateKey.export({ type: 'pkcs8', format: 'pem' }) as string;

  const daemonIdentity = ensureMobileTlsIdentity({ stateDir: path.join(TMP_ROOT, 'lic-daemon') });
  const relayIdentity = ensureMobileTlsIdentity({ stateDir: path.join(TMP_ROOT, 'lic-relay') });
  const pairId = relayPairId(daemonIdentity.certPem);

  const mintLease = (over: Record<string, unknown> = {}) => signLease(
    { ...buildLeasePayload({
      tenant: 'breakthrough', product: 'clementine', licenseId: '1', activationId: '1',
      machineIdHashHex: 'aa'.repeat(32), pairId, plan: 'pro', features: [],
      seatUsed: 1, seatLimit: 1, enforce: true,
      leaseTtlSeconds: 259_200, graceSeconds: 1_209_600,
    }), ...over },
    { privateKeyPem, kid: 'k1' },
  );

  const app = express();
  app.get('/m/ping', (_req, res) => res.json({ ok: true }));
  const listener = await startRelayInternalListener(app, {
    keyPem: daemonIdentity.keyPem, certPem: daemonIdentity.certPem,
  });
  const relay = await startRelay({
    port: 0, baseDomain: 'r.test.local',
    tlsKeyPem: relayIdentity.keyPem, tlsCertPem: relayIdentity.certPem,
    dataDir: path.join(TMP_ROOT, `lic-relay-data-${Date.now()}`),
    log: { info: () => {}, warn: () => {}, error: () => {} },
    licensePublicKeys: { k1: publicKeyBase64(privateKeyPem) },
    productSlug: 'clementine',
    requireLicense: true,
  });

  // Registers directly rather than via the client so each lease can be tested.
  const register = async (lease: string | undefined): Promise<string> => new Promise((resolve, reject) => {
    const sock = tls.connect({ host: '127.0.0.1', port: relay.port, servername: 'tunnel.r.test.local', rejectUnauthorized: false });
    const feed = frameReader();
    sock.on('secureConnect', () => {
      sock.write(encodeFrame(FRAME.HELLO, 0, JSON.stringify({ pairId, authToken: 'a'.repeat(32), proto: 1, lease })));
    });
    sock.on('data', (chunk: Buffer) => {
      for (const frame of feed(chunk)) {
        if (frame.type === FRAME.CHALLENGE) {
          const nonce = JSON.parse(frame.payload.toString('utf8')).nonce as string;
          const signature = createSign('sha256').update(nonce).sign(daemonIdentity.keyPem).toString('base64url');
          sock.write(encodeFrame(FRAME.PROOF, 0, JSON.stringify({ certPem: daemonIdentity.certPem, signature })));
          continue;
        }
        if (frame.type === FRAME.HELLO_ERR) {
          sock.destroy();
          resolve((JSON.parse(frame.payload.toString('utf8')) as { error: string }).error);
          return;
        }
        if (frame.type === FRAME.HELLO_OK) { sock.destroy(); resolve('ACCEPTED'); return; }
      }
  ...
  try {
    assert.equal(await register(mintLease()), 'ACCEPTED', 'a valid lease registers');
    assert.equal(await register(undefined), 'LICENSE_REQUIRED', 'no lease is refused when required');

    // Expired on the RELAY's clock. The daemon's 14-day grace keeps a laptop
    // working on a plane; it must not keep a lapsed subscription on the relay.
    const longAgo = Date.now() - 40 * 86_400_000;
    assert.equal(await register(mintLease(buildLeasePayload({
      tenant: 'breakthrough', product: 'clementine', licenseId: '1', activationId: '1',
      machineIdHashHex: 'aa'.repeat(32), pairId, plan: 'pro', features: [],
      seatUsed: 1, seatLimit: 1, enforce: true, leaseTtlSeconds: 60, graceSeconds: 1_209_600,
      now: longAgo,
    }))), 'LICENSE_EXPIRED', 'client grace does not extend relay access');

    // THE binding: a lease minted for a different Mac is useless here, even
    // though it is perfectly valid and signed by the real key.
    assert.equal(await register(mintLease({ pair: 'ffffffffffffffff' })), 'LICENSE_PAIR_MISMATCH');

    assert.equal(await register(mintLease({ product: 'someone-elses-fork' })), 'LICENSE_WRONG_PRODUCT');

    // Signed by a key the relay does not trust.
    const rogue = generateKeyPairSync('ed25519');
    const forged = signLease(buildLeasePayload({
      tenant: 'breakthrough', product: 'clementine', licenseId: '1', activationId: '1',
      machineIdHashHex: 'aa'.repeat(32), pairId, plan: 'enterprise', features: [],
      seatUsed: 1, seatLimit: 999, enforce: false, leaseTtlSeconds: 259_200, graceSeconds: 0,
    }), { privateKeyPem: rogue.privateKey.export({ type: 'pkcs8', format: 'pem' }) as string, kid: 'k1' });
    assert.equal(await register(forged), 'LICENSE_BAD_SIGNATURE', 'self-minted licenses are refused');
  } finally {
    await relay.close();
