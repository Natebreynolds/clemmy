...
test('the 2026-08-05 live regression: a scrape-and-sheet request aligns; the calendar lookup stays immediate', () => {
  // The v3.6 freeze removed the beat expecting the graph lane to replace it;
  // the graph was held out of HEAD, so v3.7/v3.8 shipped with no alignment at
  // all. The owner's exact message then executed tools with zero conversation.
  // This pin keeps that pair of behaviors from ever silently diverging again.
  const scrape = classifyTurnPreflight({
    message: 'i would like for you to scrape 10 personal injury law firms please and '
      + 'help me find good prospecting data and then put them into a google sheet when done. '
      + 'this needs to be good seo, identify the ideal proect based on data and scraping from apify. '
      + 'can you help me with this',
    sessionId: 'sess-live-0805',
    sessionKind: 'chat',
    isMultiItem: true,
    itemCount: 10,
    sourceUserSeq: 1,
  });
  assert.equal(scrape.phase, 'align', 'a substantial multi-item build must converse before executing');
  assert.equal(scrape.destination, 'google sheet');

  const calendar = classifyTurnPreflight({
    message: 'whats on my calender',
    sessionId: 'sess-live-0805',
    sessionKind: 'chat',
    isMultiItem: false,
    sourceUserSeq: 2,
  });
  assert.equal(calendar.phase, 'execute', 'a plain lookup must never earn a beat');
});
