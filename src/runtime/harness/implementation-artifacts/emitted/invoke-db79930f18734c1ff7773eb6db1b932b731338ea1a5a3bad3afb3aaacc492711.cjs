...
function gridFromSheetPayload(result) {
  ...
  if (Array.isArray(record.valueRanges)) {
    const first = record.valueRanges.find((entry) => entry && typeof entry === "object");
...
function canonicalizeSheetGrid(records, fields) {
  const header = fields && fields.length > 0 ? [...fields] : [...new Set(records.flatMap((record) => Object.keys(record)))];
...
function sheetArtifactFromProvider(result) {
  ...
  const handle = firstString(record, ["spreadsheet_url", "spreadsheetUrl", "url", "handle"]) || `https://docs.google.com/spreadsheets/d/${id}`;
...
async function executeSealed(operationId, args, accountId) {
  const attested = (() => {
    try {
  ...
  return async ({ payload, role, envelope, binding, authority }) => {
    ...
    if (sealed.operationId === BETA_PROVIDER_OPERATIONS.readback) {
      ...
      try {
        ...
        return {
          id,
          handle: firstString(record, ["spreadsheet_url", "spreadsheetUrl", "url"]) || void 0,
          content: normalizeSheetRows(result),
          grid: normalizeSheetGrid(result),
          range,
          receipt: firstString(record, ["receipt", "response_id", "requestId"]) || void 0,
