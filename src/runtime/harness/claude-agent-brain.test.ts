...
test('Workspace dock: native authoring stays first-class while specialized schemas remain discoverable', async () => {
  process.env.CLEMMY_TOOL_JIT = 'off';
  process.env.CLEMMY_CLAUDE_TOOL_SEARCH = 'on';
  process.env.CLEMMY_CLAUDE_AGENT_SDK_BRAIN = 'full';
  process.env.AUTH_MODE = 'claude_oauth';
  ...
  for (const common of ['space_get', 'space_get_view', 'space_edit_view', 'space_save']) {
    assert.ok(captured.mcpToolAllowlist.includes(common), `${common} stays first-class in a dock`);
  }
  for (const specialized of ['space_get_runner', 'space_edit_runner', 'space_try_runner', 'space_history', 'space_diff', 'space_action_prepare', 'space_publish']) {
