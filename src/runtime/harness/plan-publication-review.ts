...
import { awaitToolModelReview } from '../tool-abort-context.js';
...
export async function reviewPlanForPublication(candidate: PlanReviewCandidate): Promise<'continue' | 'done'> {
  const review = finalReview.getStore();
  const verdict = review ? await awaitToolModelReview(() => review(candidate)) : 'done';
