...
export interface NativeRevisionProofPorts {
  db: Database.Database;
  loadContract(input: RevisionProofInput): AcceptedTaskWorkContractV1 | null;
  loadSelection(input: RevisionProofInput): { effect: string; logicalToolName: string } | null;
  redeem(input: RevisionProofInput): { status: 'ok'; value: { executionSite: string; outcomeKind: string; toolName: string; rawPayload: unknown } }
...
export type RevisionProofInput = {
...
type RevisionProof = { status: 'verified'; handle: string; contentDigest: string; createdId: string;
  boundAt: string; settledAt: string; outputKind: string; identityLineageVerified: boolean } | { status: 'unverified'; reason: string };

export function proveNativeRevisionCommitWithPorts(input: RevisionProofInput, ports: NativeRevisionProofPorts):
  { status: 'verified'; handle: string; contentDigest: string; identityLineageVerified: boolean } | { status: 'unverified'; reason: string } {
  const proof = proveRevision(input, ports, 'current', new Set(), new Map());
  return proof.status === 'verified'
    ? { status: 'verified', handle: proof.handle, contentDigest: proof.contentDigest, identityLineageVerified: proof.identityLineageVerified } : proof;
}

function proveRevision(input: RevisionProofInput, ports: NativeRevisionProofPorts, mode: 'current' | 'historical', visiting: Set<string>, verifiedProofs: Map<string, RevisionProof>): RevisionProof {
  ...
  try {
    ...
    const loaded = contract ? { status: 'ok', contract } : null;
    if (!loaded || loaded.contract.contractId !== input.contractId
    ...
    const sealed = ports.loadSelection(input);
    ...
    const artifactIdentityLineage = operation.dataFrom.length > 0 && operation.dataFrom.every(id => {
      ...
      const prior = proveRevision({ ...input, requirementId: id, logicalToolCallId: calls[0]!.logical_tool_call_id }, ports, 'historical', visiting, verifiedProofs);
    ...
    if (operation.dataFrom.length > 0 && !artifactIdentityLineage) {
      const derivation = proveHostLocalWorkspaceDerivation({db,...input,
        ...
        resolveSuccessfulResult(logicalToolCallId) {
          const result = ports.redeem({...input,logicalToolCallId});
    ...
    verified.identityLineageVerified = artifactIdentityLineage;
    ...
    for (const successor of loaded.contract.operations) {
      ...
      const next = proveRevision({ ...input, requirementId: successor.id, logicalToolCallId: calls[0]!.logical_tool_call_id }, ports, 'current', visiting, verifiedProofs);
