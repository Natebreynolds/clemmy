...
test('nested-dispatch scopes persist and the historical code-mode suffix remains readable', () => {
  ...
  for (const scopeId of [`${sid}::nestedDispatch`, `${sid}::codeMode`]) {
  ...
  assert.doesNotThrow(() => writeGuardrailState('missing-parent::nestedDispatch', payload));
  assert.equal(readGuardrailState('missing-parent::nestedDispatch'), null);
});
