...
// --- observed internal-narration leak regression ----------------------------
// A fresh "hey hows it going" thread; user asked for 25 Salesforce prospects;
// cross-session recall surfaced the day's earlier Example Legal audits and the model
// narrated its context-hygiene check INTO the user-facing reply.

test('strips an observed stale-project "active context" preamble and keeps the question', () => {
  const leaked =
    'I checked the active context. The Salesforce prospect request is a new topic, not the stale Example Legal audit thread, so I’m not using that focus.\n\n' +
    'To pull the right 25, should I use your usual Alex-owned law-firm / priority-account prospect lane, or all Salesforce prospects untouched in the last 20 days?';
  const cleaned = scrubInternalNarration(leaked);
  assert.equal(
    cleaned,
    'To pull the right 25, should I use your usual Alex-owned law-firm / priority-account prospect lane, or all Salesforce prospects untouched in the last 20 days?',
  );
  assert.ok(!/active context|Example Legal|that focus/i.test(cleaned));
});
...
test('leaves a normal answer untouched', () => {
  const reply = 'Casey emailed you at 10:00am about the fixture renewal — want me to draft a reply?';
