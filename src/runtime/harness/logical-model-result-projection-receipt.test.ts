...
test('explicit checkpoint clipping of a nested projected result survives the next source and restart without trusting raw recall bytes', async () => {
  ...
  try {
    const idle = await compaction.compactSessionIfNeeded(session, session.toInputItems(), {
      idleMs: 73 * 60_000,
      inputBudgetTokens: 1_000_000,
      layer1RetainTurns: 1,
    });
    assert.equal(idle.result.modified, false, 'elapsed idle time cannot discard working context');
    assert.equal(idle.result.layer1.clipped, 0);
    const compacted = await compaction.compactSessionIfNeeded(session, idle.nextItems, {
      forceLayer2: true,
      inputBudgetTokens: 1_000_000,
      layer1RetainTurns: 1,
    });
    assert.ok(compacted.result.beforeTokens > 6_000);
    assert.ok(compacted.result.beforeTokens < 1_000_000 * 0.3, 'the explicit checkpoint, not token pressure, triggers clipping');
