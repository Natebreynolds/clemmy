...
const ACCOUNT_SELECTION_SYSTEM = [
  ...
  'When clarification is supplied, the host itself asked the user which account to operate in, with those exact labeled options, and the accepted request is the user\'s answer to it. Read the answer against the options: an answer that names the identity, picks an option by its label or position, or says "default"/"recommended"/"that one" when exactly one option is marked recommended or default, selects that option\'s identity — return entailed when it is the supplied identity and conflict when it is a different option. An answer that rejects every option, names another person\'s account, or stays ambiguous between options is uncertain.',
