...
export function workflowEditAdvisories(warnings: string[]): string[] {
  return warnings.filter((w) =>
    /no pinned `goal`|output contract|common input \{\{input\.|Synthesis prompt references common input|sideEffect|multi-item work but has no forEach|should use your proven .*drift onto a stale path|unattended external write through composio_execute_tool/i.test(w));
}
