...
export function buildWorkflowImprovementPrompt(input: {
  ...
  return [
    ...
    '- reviewed in-process transform (pure data shaping; no model, no tools, no approval): { id, side_effect: read, dependsOn: [<step ids>], transform: <expression> } where an expression is one of { op: "get", from: "steps.<id>.output[.<path>]" | "input.<key>" | "item[.<path>]" }, { op: "jsonParse" | "jsonStringify" | "count", value: <expression> }, { op: "literal", value: <json> }, { op: "object", fields: [{ key, value: <expression> }] }, { op: "array", items: [<expression>] }, { op: "map", value: <expression>, each: <expression over item> }, { op: "sort", value: <rows>, by: [{ column, direction: asc|desc }] }, { op: "unique", value: <rows>, keys: [..] }, { op: "select", value: <rows>, where?: { column, op: eq|ne|contains|empty|nonempty, value? }, columns?: [..], limit? }, { op: "aggregate", value: <rows>, groupBy: [..], metrics?: [{ fn: count|sum|avg|min|max, column? }] }. A reviewed CLI read returns its JSON as the step output\'s `stdout` string: parse it with { op: "jsonParse", value: { op: "get", from: "steps.<id>.output.stdout" } } and take the records with a further { op: "get" } on that transform step\'s own output.',
