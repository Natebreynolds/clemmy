    ...
    deliverOutcome(
      {
        ...
        detail: [
          ...
          `If the user wants to change the DATA (better/different rows, a tighter filter, fewer/more fields, one row per entity), first call space_get_runner('${slug}') to see which runner backs each source/action, then space_get_runner('${slug}', '<runner>') to read the exact line-numbered source. For a targeted change, use space_edit_runner('${slug}', '<runner>', [{find, replace}]); it snapshots a revert, re-pulls, and reports the new rows. For a brand-new runner or a from-scratch rewrite, write_file the runner, call space_try_runner('${slug}', '<runner>') to inspect its declared role and provenance without executing it, then space_refresh('${slug}') to request the required authority and persist an actual refresh. For layout/copy tweaks read with space_get_view then space_edit_view. Never read the view/runner or test a runner from the shell, and do NOT say it's done while the surface still shows old data.`,
        ].filter(Boolean).join('\n\n'),
      },
      {
