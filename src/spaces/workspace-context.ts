...
export function buildWorkspaceContextPrimer(slug: string): string | null {
  ...
  return [
    ...
    `Playbook: the built-in skill workspace-builder (skill_read) before creating or redesigning a surface.`,
    `Operate through Workspace tools, never shell/file-system paths. NEVER write ordinary Workspace HTML to a sandbox or scratch copy. Read manifest/data with space_get('${slug}'). For a view change, read the exact region with space_get_view('${slug}', '<nearby text>') then make a targeted space_edit_view('${slug}', [{find, replace}]). For a full ordinary rewrite, pass the self-contained HTML directly as view_html in one space_save call; view_path is only legacy/oversized compatibility. For a legacy data/query change, inspect statically with space_try_runner('${slug}', '<runner>'), edit the runner entrypoint with space_edit_runner, then call space_refresh('${slug}') (a changed entrypoint needs fresh approval). Use a provably read-only Composio source for new data pulls.`,
    `Ground completion in tool results: a saved view or empty/failed pull is not success. Use space_history('${slug}') for observation facts and space_diff('${slug}', '<source id>') for bounded changes. Never infer a delta; report insufficient history. space_action_prepare runs an ordinary declared action under the same canonical decision a click in the view uses; it stages one exact approval only for a send, delete, administrative or otherwise high-consequence effect. Workspace edits are versioned and reversible. For deferred schemas use tool_search then call_tool; report missing only after real dispatch.`,
    `Reply like a colleague: lead with the user-visible outcome, then any honest blocker or open product choice. Keep slugs, paths, and step narration internal. Never answer only "done" or tell the user to refresh—the open surface updates itself.`,
  ].join('\n\n');
}
