...
export function buildWorkspaceContextPrimer(slug: string): string | null {
  ...
  return [
    ...
    `Ground completion in tool results: a saved view or empty/failed pull is not success. Use space_history('${slug}') for observation facts and space_diff('${slug}', '<source id>') for bounded changes. Never infer a delta; report insufficient history. space_action_prepare stages exact approval, or executes only when exact standing approval covers these action/args/runner bytes. Workspace edits are versioned and reversible. For deferred schemas use tool_search then call_tool; report missing only after real dispatch.`,
