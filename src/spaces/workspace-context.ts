...
export const WORKSPACE_DOCK_TOOLS = [
  'space_get', 'space_list', 'space_edit_view', 'space_save', 'space_refresh',
] as const;
...
export function buildWorkspaceContextPrimer(slug: string): string | null {
  ...
  return [
    ...
    `CHANGE THE VIEW (layout, copy, a button, a color): ALWAYS use space_edit_view('${slug}', [{find, replace}]) — space_get('${slug}') first for the exact current text, pass ONLY the snippet that changes. Reserve write_file + space_save for a brand-new view, a from-scratch rewrite, or changing which data sources/actions exist. NEVER write the workspace HTML to a sandbox or scratch file — it only lands via space_edit_view / space_save.`,
