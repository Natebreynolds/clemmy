...
export interface ToolDecl {
  ...
  /** Where a reusable read's resource revision comes from (read-resource-
   * revision.ts). A read that declares reuse without a revision source is
   * never reused: the replay cannot tell whether another session, a run or
   * an editor changed the resource meanwhile. */
  readRevision?: 'local_path' | 'workspace_files' | 'workflow_files' | 'memory_store';
...
/** The declared revision source of a reusable read, with the argument that
 * names its resource when the source is one local path. */
export function registeredToolReadRevision(
  toolName: string,
): { source: NonNullable<ToolDecl['readRevision']>; identityArgument?: string } | null {
  const key = toolName.trim();
  if (!key) return null;
  const row = TOOL_REGISTRY.find((candidate) => candidate.name === key);
  if (!row?.readReuse || !row.readRevision) return null;
  return { source: row.readRevision, ...(row.resourceIdentityArgument ? { identityArgument: row.resourceIdentityArgument } : {}) };
}
