 ...
 *                       inner-dispatch = inner-dispatch.ts READ_ONLY_TOOLS ∪ WRITE_TOOLS
 ...
 *   - innerDispatch . `'read'` (READ_ONLY_TOOLS) or `'write'` (WRITE_TOOLS) for the
 *                     nested-dispatch allowlist; absent otherwise.
...
export type ToolLane = 'orchestrator' | 'sdk-brain' | 'sdk-worker' | 'workflow-step' | 'inner-dispatch' | 'cli';
export type ToolSideEffect = 'read' | 'write' | 'send' | 'admin';
/** Narrow runtime effect override for truthful host-internal controls whose
 * durable state write is neither a user-data write nor a pure read. */
export type ToolRuntimeEffectOverride = 'host_only';

/**
 * READS BYTES THIS SESSION ALREADY RETAINED.
 *
 * A tool marked here re-reads output the harness has already durably kept —
 * it crosses no boundary, contacts no provider, and can compound nothing no
 * matter how the previous call settled. That makes it safe on EVERY recovery
 * surface: when the last call failed, "look again at what came back" is the
 * one move that is always both useful and harmless.
 *
 * This is a registry fact rather than a list in the turn runner because the
 * runner is not the place that knows what a tool touches. A new retained
 * reader declares itself here and is admitted everywhere, with no edit to
 * recovery code.
 */
...
/** Whether a tool may run outside a frozen business-work binding on an action
 * turn. This is an explicit host taxonomy, never inferred from names, verbs,
 * providers, slugs, or the broad read/write effect class. */
export type ActionTopologyRole = 'control' | 'business';
/**
 * Durable-project authority is stricter than the legacy sideEffect taxonomy.
 * Presence means this exact built-in is both available to a constrained
 * workflow step and reviewed as replay-safe when MODEL-INVOKED in the declared
 * realm. It does not imply support for WorkflowStepInput.call; structured-call
 * execution needs its own persisted adapter contract. Absence is fail-closed:
 * planners may discover the tool but cannot put it in a compiled project node.
 */
export type ProjectToolEffect = 'read' | 'local_write';
/** Exact host execution contract for the fresh read-only chat engine. Presence
 * is a positive, reviewed declaration: a broad read/compute classification is
 * never enough to authorize a body crossing. */
export type HostReadOnlyExecutionContract = 'pure_local';
/** Model-frame scheduling is registry authority, not a tool-name convention.
 * `fresh_plan_barrier` may either own a frame alone or be followed by the one
 * narrowly admitted root read whose body remains fenced until the barrier has
 * durably settled and activated. */
export type HostModelFrameClass = 'ordinary' | 'exclusive_control' | 'fresh_plan_barrier';

/**
 * Provider-neutral semantics for a Clementine-local capability that may be
 * cited while the foreground model freezes an action plan.  This is not an
 * approval declaration.  It says only what the exact built-in can do, and is
 * deliberately stricter than the legacy read/write taxonomy: absence,
 * destructive work, and unknown reversibility are not planning authority.
 */
export type LocalPlanningConsequence =
  | 'local_artifact'
...
export interface LocalPlanningSafeMode {
  /** Stable suffix in the citable capability ref. */
  id: string;
  /** Exact scalar arguments required by this safe runtime mode. */
  requiredEquals: Readonly<Record<string, string | number | boolean>>;
  /** Required-equality fields for which schema-level null is explicitly the
   * runtime spelling of the declared scalar. This is structural authority,
   * not generic nullable coercion: the current tool schema must admit both
   * the scalar and null, and the field must also appear in requiredEquals. */
  nullEquivalentToRequired?: readonly string[];
  /** Explicit runtime default when this field is omitted; the schema must permit omission. */
  omittedEquivalentToRequired?: readonly string[];
  /** Arguments whose non-null presence would widen the declared consequence. */
  absentOrNull?: readonly string[];
  /** Scalar domains that must remain closed even when another field decides
   * the runtime branch. This is useful for precedence-style APIs: for example
   * append=true may select append independently of mode, while mode must still
   * be one of the values admitted by the configured schema. */
  allowedValues?: Readonly<Record<string, readonly (string | number | boolean | null)[]>>;
  /** Alternative structural spellings of this same runtime mode. Every branch
   * is proved against the current configured schema and exactly one declared
   * variant must match before consent/dispatch. */
  alternatives?: readonly LocalPlanningSafeModePredicate[];
}

export interface LocalPlanningSafeModePredicate {
  requiredEquals: Readonly<Record<string, string | number | boolean>>;
  nullEquivalentToRequired?: readonly string[];
  /** Explicit runtime default when this field is omitted; the schema must permit omission. */
  omittedEquivalentToRequired?: readonly string[];
...
export interface LocalPlanningSemantics {
  consequence: LocalPlanningConsequence;
  reversibility: LocalPlanningReversibility;
  destructive: boolean;
  purpose: string;
  inputKind: string;
  outputKind: string;
  ...
  /**
   * Every posture the operation ACTUALLY supports, when that is more than the
   * primary one above. An upsert is a real third case: `space_save`'s own
   * description says "Create or update a Workspace", yet its posture said
   * create_new alone, so a compatibility check reading only the primary value
   * rejected a valid save onto an existing Space. Declaring the truth here is
   * the fix; the primary posture stays the default for existing consumers.
   */
  destinationPostures?: readonly ('create_new' | 'named_existing')[];
  advisoryRoles: readonly string[];
  /** Required for a conditionally-safe operation such as write_file. */
  safeMode?: LocalPlanningSafeMode;
}

/**
 * Closed execution contract for a Clementine-local tool that may enter the
 * shared workflow call kernel. `localPlanning` alone is discovery metadata;
 * this additional declaration is the reviewed adapter boundary that says the
 * exact captured handler is safe to invoke under a frozen schema/argument
 * digest. Absence remains fail-closed.
 */
export interface ReviewedLocalExecutionContractV1 {
  version: 1;
  adapter: 'artifact_bundle_v1' | 'workspace_dataset_v1' | 'local_file_revision_v1' | 'local_file_read_v1';
  idempotency: 'content_addressed' | 'receipt_reconciled' | 'read_only';
  reconciliation: 'artifact_bundle_v1' | 'workspace_dataset_v1' | 'local_file_revision_v1' | 'none';
  /** Workflow execution freezes exact arguments before crossing. Its adapter
   * may cover multiple separately published planning modes. */
...
export interface ToolDecl {
  ...
  /** Re-reads only already-retained output; crosses nothing. See ToolRuntimeEffectOverride docs. */
  readsRetainedOutput?: boolean;
  /** Declared freshness policy for a read: an identical call in the same
   * accepted source, with no mutation settled since, may be answered from the
   * settled result (settled-read-reuse.ts). Absent = never reused: status polls
   * and anything whose state moves between reads stay physical reads. */
  readReuse?: 'settled_within_source';
  /** A host control that mutates execution authority must own its whole model
   * frame so sibling calls cannot race the state transition. */
  hostControlFrame?: 'sole';
  /** Typed host scheduling class. Absence is ordinary; unknown tools never
   * acquire a control/barrier class by spelling or wrapper shape. */
  hostModelFrameClass?: Exclude<HostModelFrameClass, 'ordinary'>;
  ...
  sdkLayer?: SdkLayer;
  /** Nested-dispatch allowlist membership + direction, if any. */
  innerDispatch?: 'read' | 'write';
  ...
  /** Exact durable-project MODEL capability/effect realm; never inferred from
   * prose. Presence does not claim a structured-call adapter exists. */
  projectEffect?: ProjectToolEffect;
  /** Argument naming the resource written by this native tool. Used to keep
   * per-item work distinct; this never grants path or effect permission. */
  resourceIdentityArgument?: string;
  /** Exact native contextual reader may also be selected as a read operation
   * in an accepted plan. Does not admit durable-project/model execution, change
   * its ordinary control role, or authorize any call without a selected binding. */
  localPlanningRead?: true;
  /** This exact built-in may execute through host_v1_read_only without an
   * external capability/account binding. Omitted is fail-closed. */
  hostReadOnlyExecution?: HostReadOnlyExecutionContract;
  /** Control/discovery calls may precede action-topology freeze. Omitted means
   * business and therefore fail-closed while action authority is active.
   * CONTROL is the whole harness-internal coordination class: staging,
   * delegation, lifecycle, memory, workflow authoring, local workspace views,
   * fan-out and approval primitives — tools whose writes are LOCAL-durable
   * and whose external consequences (if any) fire later through their own
   * gated executor. Defaulting these to business sent them through the
   * work_call proposal validator built for provider fan-out, which refused
   * approval staging, delegation, and workspace publishing outright (live
   * 2026-08-11 sweep: pending-action-gate, team-*, workspace-build). BUSINESS
  ...
  /** Exact local primitive that can execute, release, auto-test, activate, or
  ...
  /** Additional mutually-exclusive argument variants for one configured tool.
   * Unlike a broad tool-level declaration, every variant must carry a
   * structural safeMode, receives its own capability ref, and is frozen by the
   * accepted plan before work_call arguments are admitted. This permits an
   * irreversible/destructive mode to be planned truthfully without letting it
   * inherit a reversible sibling's authority. */
  localPlanningVariants?: readonly LocalPlanningSemantics[];
  /** Explicit reviewed adapter contract for the shared workflow v3 kernel.
   * This does not grant consent; the exact v3 activation still owns approval,
   * one physical crossing, settlement, and replay. */
  localExecution?: ReviewedLocalExecutionContractV1;
 ...
 * inner-dispatch surface (union of: catalog LOCAL_MCP_TOOL_NAMES, orchestrator
 * discoveryTools, the four SDK profiles, TOOL_JIT_MANDATED, nested-dispatch
...
export const TOOL_REGISTRY: ToolDecl[] = [
  ...
  { name: 'plan_step_result', sideEffect: 'write', runtimeEffect: 'host_only', hostControlFrame: 'sole', tier: 'core', lanes: [], needsApproval: false, blockedFor: ['worker'], loopClass: 'mutating', actionTopologyRole: 'control', description: 'Record model-authored intermediate output for an approved compute step; never business authority.' },
  { name: 'publish_plan', sideEffect: 'write', runtimeEffect: 'host_only', hostControlFrame: 'sole', tier: 'core', lanes: [], needsApproval: false, blockedFor: ['worker'], loopClass: 'mutating', actionTopologyRole: 'control', description: 'Publish a complete host-checked Plan mode artifact for explicit user review; never executes its proposed actions.' },
  ...
  { name: 'space_get', sideEffect: 'read', readReuse: 'settled_within_source', localPlanningRead: true, tier: 'discoverable', lanes: ['orchestrator', 'sdk-brain', 'sdk-worker'], sdkLayer: 'read-only', featureGroup: 'spaces-dock', actionTopologyRole: 'control', description: 'Read a Workspace: its manifest (title, status, data sources, re-engage contract), a snaps…' },
  ...
  { name: 'space_save', sideEffect: 'write', tier: 'discoverable', lanes: ['orchestrator', 'sdk-brain'], sdkLayer: 'authoring', featureGroup: 'spaces-dock', actionTopologyRole: 'control', delegationPrimitive: true, localPlanning: { consequence: 'workspace_definition', reversibility: 'reversible', destructive: false, purpose: 'author_workspace', inputKind: 'workspace_definition', outputKind: 'workspace_revision', deliverableKind: 'workspace', destinationPosture: 'create_new', destinationPostures: ['create_new', 'named_existing'], advisoryRoles: ['author', 'create', 'destination'] }, description: 'Create or update a Workspace; targeted edits preserve unrelated content. Static document edits can atomically update stored data, phone content and HTML with the current revision.' },
  ...
  { name: 'workflow_get', sideEffect: 'read', readReuse: 'settled_within_source', localPlanningRead: true, tier: 'discoverable', lanes: ['orchestrator', 'sdk-brain', 'cli'], sdkLayer: 'authoring', loopClass: 'idempotent', actionTopologyRole: 'control', description: 'Fetch the full definition of a single workflow by name.' },
  ...
  { name: 'workflow_unschedule', sideEffect: 'write', tier: 'discoverable', lanes: ['orchestrator', 'sdk-brain', 'cli'], sdkLayer: 'authoring', blockedFor: ['workflow-step', 'worker'], actionTopologyRole: 'control', localPlanning: { consequence: 'workflow_definition', reversibility: 'reversible', destructive: false, purpose: 'pause_workflow', inputKind: 'workflow_reference', outputKind: 'workflow_revision', deliverableKind: 'workflow', destinationPosture: 'named_existing', advisoryRoles: ['author', 'destination'] }, description: 'Disable a scheduled workflow so it stops firing, without deleting it.' },
  ...
  { name: 'workspace_roots', sideEffect: 'read', readReuse: 'settled_within_source', projectEffect: 'read', hostReadOnlyExecution: 'pure_local', tier: 'core', lanes: ['orchestrator', 'sdk-brain', 'sdk-worker', 'inner-dispatch', 'cli'], sdkLayer: 'read-only', innerDispatch: 'read', loopClass: 'idempotent', description: 'List directories Clementine is allowed to inspect or operate in.' },
...
/** inner-dispatch.ts READ_ONLY_TOOLS + WRITE_TOOLS. */
export function deriveInnerDispatchSets(): { readOnly: Set<string>; write: Set<string> } {
  return {
    readOnly: names((d) => d.innerDispatch === 'read'),
    write: names((d) => d.innerDispatch === 'write'),
  };
}
...
/** Host-declared action-topology role. Unknown/foreign tools are business by
 * default so no new tool can silently acquire pre-contract authority. */

/** Does this tool only re-read output already retained this session? */
export function toolReadsRetainedOutput(toolName: string): boolean {
  const bare = toolName.includes('__') ? toolName.slice(toolName.lastIndexOf('__') + 2) : toolName;
  return TOOL_REGISTRY.some((entry) => entry.name === bare && entry.readsRetainedOutput === true);
}
...
/** Positive registry-owned model-frame class. Foreign/provider lookalikes are
 * ordinary and therefore cannot manufacture plan/control scheduling rights. */
export function hostModelFrameClassFor(toolName: string): HostModelFrameClass {
  return TOOL_REGISTRY.find((declaration) => declaration.name === toolName)
    ?.hostModelFrameClass ?? 'ordinary';
}
...
/** Exact provider-neutral registry declaration for unpropagated immediate or
 * future execution primitives. */
export function isRegisteredDelegationPrimitive(toolName: string): boolean {
  return TOOL_REGISTRY.some((declaration) => (
    declaration.name === toolName && declaration.delegationPrimitive === true
  ));
}

/** A registry-declared local tool whose declared side effect is a pure read.
 * Under an armed action contract a read duplicates nothing, so the control
 * dispatcher may carry it directly — the admission wall's own doctrine ("A
 * READ OR COMPUTE IS NEVER GATED BY THE BINDING"). Live 2026-08-20: a mobile
 * dashboard-fix turn paid tool_search + work_call proposal grammar for
 * workflow_get / workflow_run_status / space_history — deferred READS with no
 * direct door — and spent ~a third of its wall clock on carrier ceremony. */
export function isRegistryDeclaredRead(toolName: string): boolean {
  return TOOL_REGISTRY.some((declaration) => (
    declaration.name === toolName && declaration.sideEffect === 'read'
  ));
}

/** Positive host_v1 declaration for provider-neutral built-ins whose complete
 * execution is local and read-only. Unknown names and ordinary read/compute
 * declarations deliberately return null. */
export function hostReadOnlyExecutionContractFor(
  toolName: string,
): HostReadOnlyExecutionContract | null {
  return TOOL_REGISTRY.find((declaration) => declaration.name === toolName)
    ?.hostReadOnlyExecution ?? null;
}

/** Exact registry effect for a canonical local tool identity. Unknown and
 * carrier-shaped names return null. Local-write evidence combines this with
 * the frozen manifest effect and host execution provenance; no operation-name
 * registration is needed. */
/** The declared same-source freshness policy of a read, or null when the row
 * never declared one (status polls and moving state stay physical reads). */
export function registeredToolReadReuse(toolName: string): 'settled_within_source' | null {
  const key = toolName.trim();
  if (!key) return null;
  return TOOL_REGISTRY.find((row) => row.name === key)?.readReuse ?? null;
}

export function registeredToolSideEffect(toolName: string): ToolSideEffect | null {
  return TOOL_REGISTRY.find((declaration) => declaration.name === toolName)?.sideEffect ?? null;
}
