/** C6: a submitted plan and shared vocabulary must not invoke a saved workflow.
...
for (const entry of catalog) writeWorkflow(entry.slug, {
  name: entry.name, enabled: entry.enabled, description: 'Retained C6 catalog identity; isolated inert definition.',
  steps: [{ id: 'fixture', prompt: 'Return fixture text without tools.' }],
});
const C6_REQUEST = 'Save those exact three drafts in the Outlook Drafts folder for my Scorpion mailbox. This is the live draft-write test. Keep To, Cc, and Bcc empty; preserve their subjects and bodies including punctuation and the line break; create each once. Do not send any email. Report each full subject and its full returned draft ID. Use plain-text bodies. Use a plan for these three saves, then execute it.';
...
test('retained C6 request retrieves the read canary only as advice and never redirects or queues it', () => {
  assert.equal(uniqueEnabledWorkflowMatch(C6_REQUEST)?.name, 'Outlook Scorpion Inbox Read Canary', 'the actual catalog collision remains useful regression setup');
...
test('prior workflow mentions and an unrelated current execution verb cannot manufacture a run', () => {
  const session = createSession({ kind: 'chat', channel: 'desktop' });
  appendEvent({ sessionId: session.id, turn: 1, role: 'user', type: 'user_input_received', data: { text: 'Inspect Outlook Scorpion Inbox Read Canary.' } });
  for (const text of [
    'Use a plan for the three saves, then execute it.',
    'Do not run Outlook Scorpion Inbox Read Canary. Execute this new plan instead.',
    'Inspect Outlook Scorpion Inbox Read Canary; execute the draft-saving plan.',
    'Run Outlook Scorpion Inbox Read Canary.',
  ]) {
...
test('C6 discovery does not replace requested writes with an exact workflow_run shortcut', async () => {
