...
test('typed source selection reaches the best indexed operation on the first five-row discovery page', async () => {
  ...
  try {
    for (const includeDesiredInFuzzy of [false, true]) {
      ...
      let followupHandler!: (input: unknown) => Promise<{ content: Array<{ text: string }> }>;
      registerToolSearchTool({ tool(_name: string, _description: string, _schema: unknown, callback: typeof followupHandler) { followupHandler = callback; } } as never, {
        candidateSources: [followupProvider], dispatchCarrier: 'work_call',
        async discloseForPlanning(candidates, control) {
          const staged = await providerSources.stageDisclosedPlanningProviderCandidates({
            sessionId: session.id, sourceUserSeq: continuation.seq, candidates, ...control,
          });
          return { version: 1, refs: {}, blockers: staged.blockers };
        },
      });
      const followup = JSON.parse((await followupHandler({ query, limit: 5, cursor: null,
        role_key: null, account_selection: null })).content[0]!.text);
      assert.equal(followup.results[0]?.name, operation,
