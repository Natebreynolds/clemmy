...
test('global entity ids → {{slots}} (table/app/email/domain)', () => {
  const r = slotParameterize('update tblAbc123def456 in appXyz789ghi012 for cliff@eleylawfirm.com on eleylawfirm.com');
  assert.match(r.text, /\{\{table_id\}\}/);
  assert.match(r.text, /\{\{app_id\}\}/);
  assert.match(r.text, /\{\{email\}\}/);
  assert.match(r.text, /\{\{domain\}\}/);
  assert.ok(!/tblAbc123def456|appXyz789ghi012|cliff@eleylawfirm\.com/.test(r.text), 'no concrete id survives');
...
test('GLOBAL-ONLY: a user-specific client name is NOT slotted (no user data in the global distiller)', () => {
  const r = slotParameterize('Build the Revill Law Firm outreach and the Scorpion brief');
  assert.equal(r.text, 'Build the Revill Law Firm outreach and the Scorpion brief');
  assert.deepEqual(r.slots, []);
});

test('email is slotted as one unit (its domain is not double-replaced)', () => {
  const r = slotParameterize('to a@b.com');
