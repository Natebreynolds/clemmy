...
export type MobileAccessTargetMode = 'local-preview' | 'quick' | 'custom-domain' | 'direct-app';
...
export function mobileAccessTarget(
  ...
  // No tunnel configured. If the pinned-TLS door is open, that IS the target:
  // the iOS app connects straight to this Mac and pins the daemon's own
  // certificate — end-to-end encrypted with no third party in the path.
  ...
  if (directApp) {
    ...
    return {
      ...
      qrBlockedReason,
      hardening: { cloudflareAccess: 'unknown', postureGaps: posture.gaps },
    };
  }
...
export async function generateQrSvg(hostname?: string, targetInput?: { lanIp?: string | null }): Promise<{
  svg: string;
  targetUrl: string;
  ...
  const record = readMobileAccess();
  const target = mobileAccessTarget({ record, hostnameOverride: hostname, lanIp: targetInput?.lanIp });
  if (!target.qrReady) {
