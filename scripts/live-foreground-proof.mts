...
const home = path.resolve(option('--home') ?? path.join(os.homedir(), '.clementine-next'));
...
const report: Record<string, unknown> = { version: 1, startedAt: new Date().toISOString(), baseUrl, home, turns: [] };
const turns = report.turns as Record<string, unknown>[];
function save(): void {
  mkdirSync(path.dirname(output), { recursive: true });
  writeFileSync(output, `${JSON.stringify(report, null, 2)}\n`, { mode: 0o600 });
}
async function api(url: string, body?: unknown): Promise<{ status: number; body: Record<string, unknown> }> {
...
try {
  ...
  save();
  for (const { message, expect } of messages) {
    const clientRequestId = `live-foreground-${randomUUID()}`;
    const accepted = await api('/api/harness/chat', { input: message, clientRequestId, ...(sessionId ? { sessionId } : {}) });
    if (accepted.status !== 202 || accepted.body.clientRequestId !== clientRequestId || typeof accepted.body.runId !== 'string' || typeof accepted.body.sessionId !== 'string') {
    ...
    const turn: Record<string, unknown> = { message, expect, clientRequestId, accepted: accepted.body, startedAt: new Date().toISOString() };
    turns.push(turn);
    save();
    ...
    while (Date.now() < deadline) {
      ...
      const sources = db.prepare(`SELECT seq, session_id FROM events
        ...
        AND (json_extract(data_json, '$.clientRequestId') = ? OR json_extract(data_json, '$.requestId') = ?)
        ORDER BY seq`).all(accepted.body.runId, clientRequestId, clientRequestId) as Array<{ seq: number; session_id: string }>;
