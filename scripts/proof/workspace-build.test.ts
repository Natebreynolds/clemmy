...
} from '../../src/runtime/harness/current-capability-manifest.fixture.js';
import {
...
test('workspace-build uses the supported read-only data path without weakening runner policy', () => {
  const prior = installCurrentCapabilityManifestFixtures([{
    operationId: WORKSPACE_BUILD_DATA_TOOL,
  ...
  try {
    ...
    restoreCurrentCapabilityManifestFixtures(prior);
  }
});
