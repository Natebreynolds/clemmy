...
const vectors: Vector[] = [
  ...
  { name: 'process.getBuiltinModule deleted? (direct builtin access bypasses the loader)',
    program: `return 'gbm:'+typeof process.getBuiltinModule+',lb:'+typeof process._linkedBinding;`,
    held: (r) => r.ok && r.value === 'gbm:undefined,lb:undefined' },
  { name: 'getBuiltinModule("node:child_process") → execSync(id)',
    program: `const cp = process.getBuiltinModule('node:child_process'); return cp.execSync('id').toString();`,
    held: (r) => !r.ok },
  { name: 'import node:sqlite → open a file-backed DB',
    program: `const { DatabaseSync } = await import('node:sqlite'); new DatabaseSync('${PWNED}.db'); return 'opened';`,
    held: (r) => !r.ok && /blocked/i.test(r.error ?? '') },
